...
CameraOLD::CameraOLD(const glm::vec3& position, const Settings2D& settings, float nearPlane, float farPlane)
	: m_Position(position)
...
CameraOLD::CameraOLD(const glm::vec3& position, const Settings3D& settings, float nearPlane, float farPlane)
	: m_Position(position)
	, m_NearPlane{ nearPlane }
	, m_FarPlane{ farPlane }
...
const glm::mat4& CameraOLD::View()			const { return m_View; }
const glm::mat4& CameraOLD::Projection()	const 
{
...
float			 CameraOLD::FOV()			const { return glm::degrees(m_FOV);	}
float			 CameraOLD::Aspect()		const { return m_AspectRatio;				}
float			 CameraOLD::NearPlane()		const { return m_NearPlane;					}
float			 CameraOLD::FarPlane()		const { return m_FarPlane;					}

const glm::vec3& CameraOLD::Position()		const { return m_Position;	}
const glm::vec3& CameraOLD::Front()			const { return m_Front;		}
const glm::vec3& CameraOLD::Right()			const { return m_Right;		}
const glm::vec3& CameraOLD::Up()			const { return m_WorldUp;	}


//						[SETTERS]

void CameraOLD::Position(const glm::vec3& newPosition)
{
...
void CameraOLD::Orthographic(int newWidth, int newHeight)
{
...
void CameraOLD::Perspective(const Settings3D& newSettings, float nearPlane, float farPlane)
{
	...
	m_FOV = glm::radians(newSettings.FOV);
...
void CameraOLD::FOV(float newFov)
{
	if (m_ProjectionMode == ProjectionMode::Orthographic)
...
void CameraOLD::Aspect(float newWidth, float newHeight)
{
	if (m_ProjectionMode == ProjectionMode::Orthographic)
...
void CameraOLD::ClipDistance(float newNearPlane, float newFarPlane)
{
...
void CameraOLD::LookAt(const glm::vec3& target)
{
	if (m_ProjectionMode == ProjectionMode::Orthographic)
...
void CameraOLD::Transform2D(const glm::vec3& translation, float zRotation)
{
	m_View = Transform(-translation, glm::vec3{ 1.0f }, glm::vec3{ 0.0f, 0.0f, zRotation });
}

// There can be problem with default rotation value
void CameraOLD::Transform3D(const glm::vec3& translation, const glm::vec3& rotation)
{
...
void CameraOLD::Rotate(float deltaYaw, float deltaPitch)
{
	if (m_ProjectionMode == ProjectionMode::Orthographic)
...
void CameraOLD::Zoom(float scrollOffset)
{
...
std::string CameraOLD::ToString() const
{
...
std::ostream& operator<<(std::ostream& out, const CameraOLD& camera)
{
	out << camera.ToString();
	return out;
}
...
glm::mat4 CameraOLD::Transform(glm::vec3 translation, glm::vec3 scale, glm::vec3 rotation) const
{
